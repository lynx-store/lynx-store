...
export default function AdminProductsPage() {
  ...
  // حالة نموذج اللون المضاف حالياً
  ...
  // إضافة لون للقائمة المؤقتة للمنتج
    ...
    <main className="max-w-6xl mx-auto p-6 md:p-12" dir="rtl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <form onSubmit={handleAddProduct} className="bg-gray-900 border border-gray-800 p-6 rounded-3xl space-y-4 shadow-xl">
          ...
          {/* إضافة الألوان بصورها ومقاساتها */}
          <div className="border border-gray-800 p-3 rounded-2xl space-y-3 bg-gray-950/50">
            <p className="text-xs font-bold text-amber-400">إضافة خيارات الألوان والمقاسات والصور:</p>
            <div className="flex gap-2">
              <input
                ...
                className="w-1/2 bg-gray-900 border border-gray-800 rounded-xl p-2 text-xs text-white"
              />
              <input
                type="color"
                value={colorInput.color_hex}
                onChange={(e) => setColorInput({ ...colorInput, color_hex: e.target.value })}
                className="w-1/2 h-9 bg-gray-900 border border-gray-800 rounded-xl cursor-pointer"
              />
            </div>
            <input
              type="url"
              ...
              className="w-full bg-gray-900 border border-gray-800 rounded-xl p-2 text-xs text-white"
            />
            <button
              type="button"
              onClick={addColorToProduct}
              className="w-full bg-gray-800 hover:bg-gray-700 text-white font-bold py-2 rounded-xl text-xs"
            >
              + اعتماد هذا اللون للمنتج
            </button>

            {/* قائمة الألوان المضافة حالياً */}
            <div className="flex flex-wrap gap-2 pt-2">
              {formData.colors.map((c, i) => (
                <span key={i} className="bg-gray-800 border border-gray-700 text-xs text-amber-400 px-2 py-1 rounded-lg flex items-center gap-1">
                  <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: c.color_hex }}></span>
                  {c.color_name}
                </span>
              ))}
            </div>
              ...
              <div key={p.id} className="bg-gray-900 border border-gray-800 p-4 rounded-2xl space-y-3">
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400">الألوان المتاحة: {p.colors?.length || 0}</p>
                  </div>
                </div>
